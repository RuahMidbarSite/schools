...
4. If a field is missing: return "" 
(empty string) for text fields, return 0 for number fields, and return null for the Date field.
5. Contextual Disambiguation of 'ב' and 'ל': 
   - The prefix 'ל' (to/for) usually indicates a target, such as a school or audience (e.g., 'תוכנית לעתידים' -> RawSchoolName='עתידים'). 
   - The prefix 'ב' (in/at) is highly ambiguous. For example, "תוכנית בלחימה" means the topic is combat (Plan="לחימה"), while "תוכנית בעתידים" means the location is Atidim (RawSchoolName="עתידים"). 
   - RULE TO RESOLVE AMBIGUITY: First, check if the word after 'ב' is an academic subject, a skill, or exists in your known [${plansListString}] list. If it is, classify it as 'Plan'. If it is a proper noun, a location, or accompanied by words like 'בי"ס', classify it as 'RawSchoolName'.
...
8. Clean Data Extraction: For all fields, extract only the core value. Remove "filler" words like "בית ספר", "ביה"ס", "תוכנית", "תכנית", "העיר", "אזור", or "סוג". Strip prepositions like 'ל' or 'ב' from the beginning of extracted names ONLY AFTER you have classified them (e.g., "לעתידים" -> "עתידים", "בלחימה" -> "לחימה").
  Examples: 
  - Instead of "תוכנית מנהיגות", return "מנהיגות".
  - Instead of "בית ספר אגמים", return "אגמים".
  - Instead of "אזור הדרום", return "דרום".
  - Instead of "לעתידים", return "עתידים".

The JSON object should have exactly the following structure:
{
  "RawSchoolName": "School name in Hebrew. Enter only the school name without the city name and strip prefixes like 'ל' or 'ב' (e.g., for 'לעתידים' return 'עתידים')",
  "City": "City name in Hebrew (e.g., 'חדרה')",
  "ProgramName": "The explicit name of the program if provided. Do NOT use the target school as the program name (e.g., in 'תוכנית לעתידים', 'עתידים' is NOT the program name). If no explicit name is given, this MUST be exactly the same as the 'Plan' field. NEVER put statuses like 'חדש', 'פעיל' or dates in this field. (e.g., 'מוסיקה')",
  "Weeks": "The number of weeks the program will run. if there is no explicit number, return \"\" (e.g., 30)",
  ...
  "Date": "ISO date YYYY-MM-DD. Recognize dates even if the word 'תאריך' is missing (e.g., '1/4/26' or 1.4.26). Handle single-digit days and months by adding leading zeros (e.g., '1/4/26' -> '2026-04-01'). If missing, return null.",
