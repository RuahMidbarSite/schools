...
export async function sendMessageViaWhatsApp(
  ...
  try {
    ...
    if (contactData?.RoleName) formData.append("RoleName", contactData.RoleName);
    if (contactData?.SchoolName) formData.append("SchoolName", contactData.SchoolName);
    if (contactData?.CityName) formData.append("CityName", contactData.CityName);
